...
const User = () => {
  ...
  const getAccount = async () => {
    ...
    const cooperantsAccountsBalance = await coopContract.getUserAccountBalance(
      accountOne
    );
    setCooperantBalance(parseInt(cooperantsAccountsBalance[2]));
    setTotalRaspberies(parseInt(cooperantsAccountsBalance[1]));
  };
  ...
  const depositRaspberry = async (props) => {
    ...
    const done_zero = coopContract.depositFruitsToCOOP(props);
    console.log(done_zero);

    var done = coopContract.depositFruitsToCOOP(
      props,
